...
| `whisper.device` | 识别设备，默认 `auto`：优先尝试 NVIDIA GPU，不可用会降级 CPU 并提示原因；也可手动选 `cpu` 或 `cuda` |
| `whisper.compute_type` | 计算精度，默认 `auto`：CPU 使用 int8，CUDA 会依次尝试 float16、int8_float16、float32，失败后降级 CPU |
| `whisper.auto_cpu_threads` | 是否按 CPU 核心数自动选择识别线程，默认开启；关闭后使用 `whisper.cpu_threads` |
...
- 在齿轮设置里把“响应模式”切到“极速 / 游戏性能”或“均衡”；极速会使用 CPU int8、2 线程、单 worker、翻译单并发，并默认使用 base 级性能模型，优先保护 PUBG、APEX、Valorant 等竞技游戏的帧时间
- 检查网络连接
