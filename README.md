...
如果使用 Release 里的免安装包，解压后直接运行 `VoxGo.exe`。v0.3.1 起提供 3 个包：

| 包 | 内容 | 适合谁 |
|----|------|--------|
| Lite | 不带 Whisper 模型，不带 CUDA DLL，体积最小；首次运行按需下载模型 | 大多数用户、网络能下载模型 |
| Full | 内置 Whisper small 模型，不带 CUDA DLL | 网络不稳定、不想首次下载模型的 CPU 用户 |
| Full-CUDA / GPU 版 | 内置 Whisper small 模型和 CUDA DLL | NVIDIA 显卡用户，想直接使用 GPU 识别 |
...
- full 包已内置 Whisper small 和 base 模型，默认游戏性能模式不需要首次下载模型
...
## 📝 注意事项
1. Lite 首次运行会下载 Whisper 模型；Full/Full-CUDA 已内置 Whisper small 模型
2. 需要稳定的网络连接用于翻译 API
3. 建议在游戏前启动本程序
4. 手机端需要保持浏览器页面打开
